...
if (isset($_POST["submit"]) && !empty($_POST["search"])) {
    ...
    $sql = "SELECT * FROM employees WHERE id LIKE '%$search%' OR firstName LIKE '%$search%' OR lastName LIKE '%$search%' OR birthDay LIKE '%$search%' OR gender LIKE '%$search%' OR country LIKE '%$search%' OR `address` LIKE '%$search%' OR position LIKE '%$search%'OR email LIKE '%$search%'";
} else {
...
$total_records_query = "SELECT COUNT(*) as count FROM employees";
...
<body>
  <header>
    <nav class="navbar navbar-expand-lg navbar-dark fs-5">
      <div class="container">
        <a class="navbar-brand fs-4" href="#">Art Gallery</a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          <ul class="navbar-nav">
            ...
            <li class="nav-item">
              <a class="nav-link" href="../artist/artistTable.php">Artists</a>
            </li>
            <li class="nav-item">
            ...
            <li class="nav-item">
              <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
                <input class="nav-link active" type="submit" name="employeeTable" value="Employees">
              </form>
            </li>
          </ul>
          ...
          <ul class="navbar-nav ms-auto">
            <li class="nav-item">
              <a class="nav-link" href="employeeProfile.php">Your Profile</a>
            </li>
            <li class="nav-item">
  ...
  <main>
    <div class="container mt-5 mb-3">
      <div class="row mx-2 rounded-top-2">
        <!-- display alert -->
        <?php
        if (isset($_SESSION['employeeTableAccess'])) {
        ?>
          <div class="alert alert-success mb-0 rounded-bottom-0" role="alert">
            <div>
            <i class="bi bi-person-check me-1"></i><?php echo $_SESSION['employeeTableAccess']; ?>
            </div>
          </div>
        <?php
          unset($_SESSION['employeeTableAccess']);
        }
        else if (isset($_SESSION['deleteEmployeeNotification'])) {
        ?>
          <div class="alert alert-warning mb-0 rounded-bottom-0" role="alert">
            <div>
              <i class="bi bi-person-x me-1"></i><?php echo $_SESSION['deleteEmployeeNotification']; ?>
            </div>
          </div>
        <?php
          unset($_SESSION['deleteEmployeeNotification']);
        } 
        else if (isset($_SESSION['updateEmployeeNotification'])) {
        ?>
          <div class="alert alert-info mb-0 rounded-bottom-0" role="alert">
            <div>
              <i class="bi bi-person-gear me-1"></i><?php echo $_SESSION['updateEmployeeNotification']; ?>
            </div>
          </div>
        <?php
          unset($_SESSION['updateEmployeeNotification']);
        }
        ?>

        <h2 class="mb-3 mt-2">Employee Table</h2>
        <!-- search bar -->
        <div class="col">
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        ...
        <!-- items per page dropdown & add new employee button -->
        <div class="col text-end">
          <a href="newEmployee.php" class="btn btn-outline-color mb-1 mb-lg-0">Add new member<i class="bi bi-plus-circle ms-2"></i></a>
          <div class="btn-group">
              ...
              <tr>
                ...
                <td>
                  <div class="btn-group gap-1">
                    <a href="readEmployee.php?id=<?= $employee['id'] ?>" class="btn btn-outline-secondary"><i class="bi bi-binoculars"></i></a>
                    <a href="updateEmployee.php?id=<?= $employee['id'] ?>" class="btn btn-outline-success"><i class="bi bi-pencil-square"></i></a>
      ...
      <div class="row mx-2 pt-3 rounded-bottom-2">
        ...
        <nav class="d-flex justify-content-center">
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link" href="employeeTable.php?page=<?= $previous ?>"><i class="bi bi-chevron-double-left"></i></a>
            </li>
            <?php for ($i = 1; $i <= $pages; $i++) : ?>
              <li class="page-item"><a class="page-link" href="employeeTable.php?page=<?= $i ?>"><?= $i ?></a></li>
            <?php endfor; ?>
            <li class="page-item">
              <a class="page-link" href="employeeTable.php?page=<?= $next ?>"><i class="bi bi-chevron-double-right"></i></a>
            </li>
          </ul>
        </nav>
      </div>
