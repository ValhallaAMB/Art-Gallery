...
if (isset($_POST["submit"]) && !empty($_POST["search"])) {
    ...
    $sql = "SELECT * FROM guests WHERE id LIKE '%$search%' OR firstName LIKE '%$search%' OR lastName LIKE '%$search%' OR email LIKE '%$search%' OR dateOfAttendance LIKE '%$search%' OR paidOrInvited LIKE '%$search%' OR phoneNumber LIKE '%$search%' OR plusOne LIKE '%$search%'";
} else {
    $sql = "SELECT * FROM guests LIMIT $offset, $limit";
}
...
$total_records_query = "SELECT COUNT(*) as count FROM guests";
...
// to view the employee table only if the user is a manager or marketing manager
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="guestTable.php">Guests</a>
                        </li>
                        <li class="nav-item">
    ...
    <main>
        <div class="container mt-5 mb-3">
            <div class="row mx-2 rounded-top-2">
                <!-- display alert -->
                <?php
                if (isset($_SESSION['deleteGuestNotification'])) {
                ?>
                    <div class="alert alert-warning mb-0 rounded-bottom-0" role="alert">
                        <div>
                            <i class="bi bi-person-x me-1"></i><?php echo $_SESSION['deleteGuestNotification']; ?>
                        </div>
                    </div>
                <?php
                    unset($_SESSION['deleteGuestNotification']);
                } 
                else if (isset($_SESSION['updateGuestNotification'])) {
                ?>
                    <div class="alert alert-info mb-0 rounded-bottom-0" role="alert">
                        <div>
                            <i class="bi bi-person-gear me-1"></i><?php echo $_SESSION['updateGuestNotification']; ?>
                        </div>
                    </div>
                <?php
                    unset($_SESSION['updateGuestNotification']);
                } 
                ...
                <h2 class="mb-3 mt-2">Guest Table</h2>
                <!-- search bar -->
                <div class="col">
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                ...
                <!-- items per page dropdown & add new guest button -->
                <div class="col text-end">
                    <a href="newGuest.php" class="btn btn-outline-color mb-1 mb-lg-0">Add new guest<i class="bi bi-plus-circle ms-2"></i></a>
                    <div class="btn-group">
                ...
                <table class="table table-hover" id="example">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Paid or Invited</th>
                    ...
                    <tbody class="table-group-divider">
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?= $guest['dateOfAttendance'] ?></td>
                                <td><?= $guest['paidOrInvited'] ?></td>
                                <td><?= $guest['plusOne'] ?></td>
                                <td>
                                    <div class="btn-group gap-1">
                                        <a href="readGuest.php?id=<?= $guest['id'] ?>" class="btn btn-outline-secondary"><i class="bi bi-binoculars"></i></a>
                                        <a href="updateGuest.php?id=<?= $guest['id'] ?>" class="btn btn-outline-success"><i class="bi bi-pencil-square"></i></a>
            ...
            <div class="row mx-2 pt-3 rounded-bottom-2">
                ...
                <nav class="d-flex justify-content-center">
                    <ul class="pagination">
                        <li class="page-item">
                            <a class="page-link" href="guestTable.php?page=<?= $previous ?>"><i class="bi bi-chevron-double-left"></i></a>
                        </li>
                        <?php for ($i = 1; $i <= $pages; $i++) : ?>
                            <li class="page-item"><a class="page-link" href="guestTable.php?page=<?= $i ?>"><?= $i ?></a></li>
                        <?php endfor; ?>
                        <li class="page-item">
                            <a class="page-link" href="guestTable.php?page=<?= $next ?>"><i class="bi bi-chevron-double-right"></i></a>
                        </li>
                    </ul>
                </nav>
            </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title"> Delete Guest Data </h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <form action="deleteGuests.php" method="POST">
                        <div class="modal-body">
                            <input type="hidden" name="id" id="delete_id">
                            <h6>Do you want to delete this guest?</h6>
                        </div>
